fetch("http://localhost:3000/api/figures")
  ...
  .then((allFigureObjects) => {
    ...
    for (let figureObject of allFigureObjects) {
      if (
        +figureObject.y > canvas.height / 2 ||
        +figureObject.y < -(canvas.height / 2)
      ) {
        ctx.scale = 0.9;
      }
    ...
    function paintRegularTriangle(figureObject) {
      ...
      ctx.translate(
        +figureObject.x + height / 2,
        -(+figureObject.y + height / 2)
      );

      ctx.moveTo(+figureObject.x, +figureObject.y);
      ctx.lineTo(
        +figureObject.x + +figureObject.a / 2,
        +figureObject.y + height
      );
      ctx.lineTo(
        +figureObject.x - +figureObject.a / 2,
        +figureObject.y + height
    ...
    function paintRightTriangle(figureObject) {
      ...
      ctx.lineTo(
        +figureObject.x + +figureObject.b,
        +figureObject.y + +figureObject.a
      );
    ...
    function paintCircle(figureObject) {
      ...
      ctx.strokeStyle = figureObject.conturColour;
      ctx.lineWidth = 1;
      ctx.arc(figureObject.x, figureObject.y, figureObject.r, 0, 2 * Math.PI);
      ctx.fillStyle = figureObject.bgColour;
      ctx.fill();
      ctx.stroke();
      ctx.restore();
